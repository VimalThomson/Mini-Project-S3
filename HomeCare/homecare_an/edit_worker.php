...
$su_id=$_GET['s_id'];
$result="SELECT * FROM `users` join `role_db` on users.role_id=role_db.role_id join `worker` on worker.role_id=role_db.role_id where role_db.role_id='$su_id'";
$add=mysqli_query($con,$result);
 ...
 if(isset($_POST['submit']))
 {
    ...
    $wage=$_POST['work_age'];
    $wgender=$_POST['work_gender'];

    mysqli_query($con, "UPDATE `role_db` SET `email`='$wemail', `password`='$wpswd' WHERE role_id='$su_id'");   
    mysqli_query($con,"UPDATE `worker_registration_db` SET `w_name`='$name', `w_phone`='$wphn', `w_place`='$wolace', `worker_service`='$wserv' , `w_age`='wage', `w_gender`='wgender'
    WHERE role_id='$su_id'");
...
<h2 id="title" class="title">Edit Workers  </h2>
  ...
  <form method="POST"><div id="survey-form">
      ...
      <?php 
        ...
        <input id="name" type="text" placeholder="Enter worker name" name="work_name" value="<?php echo $row['fname'];?>" required>
        <label for="type" id="password-label">* Mobile number:</label>
        <input id="text" type="number" placeholder="Enter worker number" name="work_phn" value="<?php echo $row['phone'];?>" required>
        ...
        <input id="text" type="text" placeholder="Enter worker location" name="work_place" value="<?php echo $row['place'];?>" required>
